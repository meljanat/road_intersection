...
async fn main() {
    ...
    loop {
        ...
        if is_key_down(KeyCode::Up) {
            if vehicles_up.len() == 0 || vehicles_up[vehicles_up.len() - 1].y <= 750 {
                vehicles_up.push(Vehicle::new(405, 800, 3, Direction::Up));
            }
        } else if is_key_down(KeyCode::Down) {
            if vehicles_down.len() == 0 || vehicles_down[vehicles_down.len() - 1].y >= 50 {
                vehicles_down.push(Vehicle::new(355, 0, 3, Direction::Down));
            }
        } else if is_key_down(KeyCode::Left) {
            if vehicles_left.len() == 0 || vehicles_left[vehicles_left.len() - 1].x <= 750 {
                vehicles_left.push(Vehicle::new(800, 355, 3, Direction::Left));
            }
        } else if is_key_down(KeyCode::Right) {
            if vehicles_right.len() == 0 || vehicles_right[vehicles_right.len() - 1].x >= 50 {
                vehicles_right.push(Vehicle::new(0, 405, 3, Direction::Right));
            }
        } else if is_key_pressed(KeyCode::R) {
            let direction = Direction::random();
            match direction {
                Direction::Up =>  if vehicles_up.len() == 0 || vehicles_up[vehicles_up.len() - 1].y <= 710 {
                vehicles_up.push(Vehicle::new(405, 800, 3, Direction::Up));
            },
                Direction::Down =>  if vehicles_down.len() == 0 || vehicles_down[vehicles_down.len() - 1].y >= 90 {
                vehicles_down.push(Vehicle::new(355, 0, 3, Direction::Down));
            },
                Direction::Left =>  if vehicles_left.len() == 0 || vehicles_left[vehicles_left.len() - 1].x <= 710 {
                vehicles_left.push(Vehicle::new(800, 355, 3, Direction::Left));
            },
                Direction::Right =>  if vehicles_right.len() == 0 || vehicles_right[vehicles_right.len() - 1].x >= 90 {
                vehicles_right.push(Vehicle::new(0, 405, 3, Direction::Right));
            }
            };
        }
